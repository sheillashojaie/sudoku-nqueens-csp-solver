        ...
        for assignment in self.satAssignments:
            ...
            for i, v in enumerate(self.scope()):
                if i != vindex and not v.inCurDomain(assignment[i]):
                    found = False  # This assignment didn't work it assigns
...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        self.satAssignments = []
        for vali in range(self.n):
            vali += 1
            for valj in range(self.n):
                valj += 1
                diag = abs(vali - valj) == abs(self.i - self.j)
                if not diag and vali != valj:
                    self.satAssignments.append([vali, valj])
        TableConstraint.__init__(self, self._name, scope, self.satAssignments)
...
class NValuesConstraint(Constraint):
    ...
    def hasSupport(self, var, val):
        ...
        def valsNValues(l):
            vals = [val for (var, val) in l if val in self._required]
            return (len(vals) >= self._lb) and (len(vals) <= self._ub)
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], valsNValues, valsNValues)
        return x
